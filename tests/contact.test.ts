...
describe("Contact Controller", () => {
  ...
  test("POST /contacts - should create a new contact", async () => {
    ...
    (contactServiceMock.createContact as jest.Mock).mockResolvedValue(mockContact);
  ...
  test("GET /contacts - should return a list of contacts", async () => {
    ...
    (contactServiceMock.getContacts as jest.Mock).mockResolvedValue(mockContacts);
  ...
  test("GET /contacts/:id - should return a contact by ID", async () => {
    ...
    (contactServiceMock.getContactById as jest.Mock).mockResolvedValue(mockContact);
  ...
  test("PUT /contacts/:id/status - should update contact status", async () => {
    ...
    (contactServiceMock.updateContactStatus as jest.Mock).mockResolvedValue(updatedContact);

    const response = await request(app).put(`/contacts/${updatedContact._id}/status`).send({ status: "active" });
  ...
  test("PUT /contacts/:id/status - should return 404 if contact not found", async () => {
    (contactServiceMock.updateContactStatus as jest.Mock).mockResolvedValue(null);

    const response = await request(app).put("/contacts/99/status").send({ status: "inactive" });
