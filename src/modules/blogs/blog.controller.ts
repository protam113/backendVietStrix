...
export const createBlogData = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    console.error("Error creating blog:", error instanceof Error ? error.stack : error);
...
export const getBlogs = async (req: Request, res: Response) => {
  try {
    ...
    console.error("Error fetching blog:", error instanceof Error ? error.stack : error);
    res.status(500).json({ message: "Internal Server Error" });
  }
};

export const getBlogDataBySlug = async (req: Request, res: Response): Promise<void> => {
    try {
