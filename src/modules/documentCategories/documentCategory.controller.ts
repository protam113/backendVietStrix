...
export const setDocumentCategoryService = (service: DocumentCategoryService) => {
    documentCategoryService = service; // Cho phép thay thế service khi test
};


export const createDocCategory = async (req: Request, res: Response): Promise<void> => {
    try {
        const newDocumentCategory = await documentCategoryService.createDocumentCategory(req.body);
        res.status(201).json({ message: "Document category created successfully", data: newDocumentCategory });
    } catch (error: any) {
...
export const getDocumentCategories = async (req: Request, res: Response) => {
  try {
    ...
    console.error("Error fetching documentCategories:", error instanceof Error ? error.stack : error);
    res.status(500).json({ message: "Internal Server Error" });
  }
};

export const getDocumentCategoriesById = async (req: Request, res: Response): Promise<void> => {
    try {
      const documentCategory = await documentCategoryService.getDocumentCategoryById(req.params.id);
      if (!documentCategory) {
...
export const updateDocumentCategory = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const updatedDocumentCategory = await documentCategoryService.updateDocumentCategoryID(req.params.id, name, slug);
      ...
      res.status(200).json({ message: "DocumentCategory updated", data: updatedDocumentCategory });
    } catch (error) {
...
export const deleteDocumentCategory = async (req: Request, res: Response): Promise<void> => {
    try {
      const deletedDocumentCategory = await documentCategoryService.deleteDocumentCategory(req.params.id);
