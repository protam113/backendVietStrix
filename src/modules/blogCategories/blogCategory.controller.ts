...
export const createBlogCategory = async (
  ...
  try {
    const newBlogCategory = await blogCategoryService.createBlogCategory(
      req.body
    );
    res.status(201).json({
...
export const getBlogCategoriesById = async (
  req: Request,
  res: Response
): Promise<void> => {
  try {
...
export const updateBlogCategory = async (
  ...
  try {
    ...
    const updatedBlogCategory = await blogCategoryService.updateBlogCategoryID(
      req.params.id,
      name,
      slug
    );
    ...
    res
      .status(200)
      .json({ message: 'BlogCategory updated', data: updatedBlogCategory });
  } catch (error) {
...
export const deleteBlogCategory = async (
  ...
  try {
    const deletedBlogCategory = await blogCategoryService.deleteBLogCategory(
      req.params.id
    );
